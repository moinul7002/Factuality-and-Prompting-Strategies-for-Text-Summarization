...
def async_process(fn,inps,workers=10,msg='Breaking down into atomic facts.'):
...
class AtomicFactGenerator(object):
    def __init__(self, key_path, demon_dir, gpt3_cache_file=None,model_name = 'InstructGPT',n=7):
    ...
    def get_init_atomic_facts_from_sentence(self, sentences, cost_estimate=None):
        ...
        def break_down_facts(prompt,prompt_to_sent):
            output,_ = self.openai_lm.generate(prompt,few_shot=True)
            if output is not None:
                return {prompt_to_sent[prompt]:text_to_sentences(output)}
            else:
                return None
        ...
        for sentence in sentences:
            ...
            for i in range(self.n):
                ...
                joined_facts = '\n'.join(["- {}".format(fact) for fact in demons[list(demons.keys())[i]]])
                prompt = prompt + joined_facts + "\n\n"

            for match in top_machings:
                prompt = prompt + "Please breakdown the following sentence into independent facts: {}\n".format(match)
                joined_facts = '\n'.join(["- {}".format(fact) for fact in demons[match]])
                prompt = prompt + joined_facts + "\n\n"

            prompt = prompt + "Please breakdown the following sentence into independent facts: {}".format(sentence)
        ...
        if cost_estimate:
            ...
            ## Do asynch here.
            atom_fact_fn = partial(break_down_facts,prompt_to_sent=prompt_to_sent)
            atom_gens = async_process(atom_fact_fn,prompts,workers=20)
            atom_gens = [a for a in atom_gens if a is not None]
            for a in atom_gens:
                atoms.update(a)
